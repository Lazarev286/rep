...
class InsGeneralLedger(models.TransientModel):
    ...
    def build_detailed_move_lines_xlsx(self, offset=0, account=0, fetch_range=FETCH_RANGE):
        '''
        ...
        WHERE_CURRENT = WHERE + " AND l.date >= '%s'" % data.get('date_from') + " AND l.date <= '%s'" % data.get(
            'date_to')
        WHERE_CURRENT += " AND a.code = '%s'" % account
        ...
        if data.get('initial_balance'):
            ...
            WHERE_FULL = WHERE + " AND l.date >= '%s'" % data.get('date_from') + " AND l.date <= '%s'" % data.get(
                'date_to')
        WHERE_FULL += " AND a.code = '%s'" % account
    ...
    def action_xlsx(self):
        ...
        #     'report_type': 'xlsx'
        # }
        return self.env.ref('account_dynamic_reports.ins_general_ledger_xlsx').report_action(self)
