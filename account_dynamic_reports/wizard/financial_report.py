...
class InsFinancialReport(models.TransientModel):
        ...
        if accounts:
            ...
            if self.account_report_id != \
                ...
                if report.type in ['accounts','account_type'] and report.range_selection == 'current_date_range':
                    if (self.date_to and self.date_from) or (date_from and date_to):
                        context.update({'strict_range': True, 'initial_bal': False, 'date_from': date_from or self.date_from,'date_to': date_to or self.date_to})
                    else:
                ...
                if report.type in ['accounts','account_type'] and report.range_selection == 'initial_date_range':
                    if date_from or self.date_from:
                        context.update({'strict_range': True, 'initial_bal': True, 'date_from': date_from or self.date_from,'date_to': False})
                    else:
        ...
        for report in reports:
            ...
            if report.type == 'accounts':
                ...
                if self.account_report_id != \
                    ...
                    res[report.id]['account'] = self.with_context(self.env.context)._compute_account_balance(report.account_ids, report)
    ...
    def get_account_lines(self, data):
        ...
        if self.account_report_id == \
                self.env.ref('account_dynamic_reports.ins_account_financial_report_cash_flow0') or self.force_show_summary:
        ...
        if data['enable_filter'] or (data.get('comparison_context').get("date_to_cmp") and data.get('comparison_context').get("date_from_cmp")):
    ...
    def get_report_values(self):
        ...
        data['form'] = self.read(
            ['date_from', 'enable_filter', 'analytic_group','debit_credit', 'date_to', 'date_range',
        ...
        if self.enable_filter:
            ...
            data['form']['analytic_group'] = False
        ...
        if data['form']['analytic_group'] and self.analytic_ids:
            data['form']['used_context']['analytic_name'] = []
            data_temp = data
            report_lines, initial_balance, current_balance, ending_balance = self.get_account_lines(data_temp.get('form'))
            report_lines = []
            for analytic in self.analytic_ids:
                data_temp['form']['used_context']['analytic_account_ids'] = analytic
                report_lines_temp, initial_balance_temp, current_balance_temp, ending_balance_temp = self.get_account_lines(data_temp.get('form'))
                data['form']['used_context']['analytic_name'].append(analytic.display_name) 
                for o in report_lines_temp:
                    o['analytic'] = analytic.display_name
                report_lines = report_lines + report_lines_temp
        ...
        if self.absolute_summary:
            data['initial_balance'] = abs(initial_balance) or 0.0
            data['current_balance'] = abs(current_balance) or 0.0
            data['ending_balance'] = abs(ending_balance) or 0.0
        else:
    ...
    analytic_group = fields.Boolean(
        string='Grouping by Analytics',
        default=True,
        help="Help to grouping the report by analytic account.")
    absolute_summary = fields.Boolean(
        string='Absolute Sum Balance?',
        default=True,
        help="Helps to display summary footer with absolute amounts.")
    force_show_summary = fields.Boolean(
        string='Force Show Summary',
        default=False,
        help="By default, only cash flow report have summary footer.")
    ...
    @api.onchange("enable_filter", "date_from_cmp", "date_to_cmp")
    def onchange_compare_filter(self):
    ...
    def action_xlsx(self):
        ...
        #     }
        # except:
        return self.env.ref('account_dynamic_reports.ins_financial_report_xlsx').report_action(self,)
