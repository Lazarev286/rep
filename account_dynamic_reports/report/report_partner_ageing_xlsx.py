...
class InsPartnerAgeingXlsx(models.AbstractModel):
    ...
    def prepare_report_filters(self, filter):
        ...
        if filter:
            ...
            sheet_2.write_string(row_pos_2, 0, _('As on Date'),
                                    format_header)
            sheet_2.write_datetime(row_pos_2, 1, convert_to_date(str(filter['as_on_date']) or ''),
                                        content_header_date)
                    ...
                    if line != 'Total':
                        ...
                        for sub_line in sub_lines:
                            ...
                            sheet.write_string(row_pos, 0, sub_line.get('move_name') or '',
                                                    line_header_light)
                            date = convert_to_date(sub_line.get('date_maturity') and sub_line.get('date_maturity').strftime('%Y-%m-%d') or sub_line.get('date').strftime('%Y-%m-%d'))
    ...
    def generate_xlsx_report(self, workbook, data, record):
        ...
        line_header_light_period.num_format = self.env.user.company_id.currency_id.excel_format
        line_header_total.num_format = self.env.user.company_id.currency_id.excel_format
        ...
        if record:
            ...
            if filters:
                ...
                # sheet_2.write_datetime(row_pos_2, 1, convert_to_date(str(filter['as_on_date']) or ''),
                #                             content_header_date)
                sheet_2.write_string(row_pos_2, 1, str(datetime),content_header_date)
                ...
                sheet_2.write_string(row_pos_2, 0, _('Partner Tag'),
                                        format_header)
                p_list = ', '.join([lt or '' for lt in filters.get('categories')])
                sheet_2.write_string(row_pos_2, 1, p_list,
                                        content_header)
            # Content section
        data = data[0]
                ...
                if record.include_details:
                    if line != 'Total':
                        ...
                        for sub_line in sub_lines:
                            ...
                            sheet.write_string(row_pos, 0, sub_line.get('move_name') or '',
                                                    line_header_light)
                            datestring = fields.Date.from_string(sub_line.get('date_maturity') and sub_line.get('date_maturity').strftime('%Y-%m-%d') or sub_line.get('date').strftime('%Y-%m-%d')).strftime(language_id.date_format)
